...
$sql = $con->query("SELECT tencay FROM db_trees WHERE Loaicay='Ăn quả' ");


echo "<form action=chitiet.php method=GET>";
echo '<table frame="border" border=4>';
echo "<tr id='tr'><th>Tên cây </th><th>Lựa chọn</th>";
foreach ($sql = $con->query("SELECT Mact,Tencay,Loaicay FROM db_trees WHERE Loaicay='Ăn quả' ") as $value){
    echo "<tr id='tr'>
